...
詳細は [SKILLS.md](../SKILLS.md) を参照。以下は特に重要なポイント:

- **テンポは速めに**: アニメーションの初期値は想像より速めに設定する
- **インパクト重視**: サイズ・回転は大胆に振る。画面からはみ出すくらいでOK
- **`random()` を `draw()` で使わない**: 毎フレームちらつく原因。代わりに `noise()` + 時間軸で滑らかに変化させる
- **彩度は高めに**: saturation 75〜100 を積極的に使う
- **duration はコンテンツから逆算**: 余分な黒フレームを作らない
